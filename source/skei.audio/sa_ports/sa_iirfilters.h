#ifndef sa_iirfilters_included
#define sa_iirfilters_included
//----------------------------------------------------------------------
...
private:
  ...
  __MIP_ALIGNED(MIP_ALIGNMENT_SIMD)
...
const clap_plugin_descriptor_t sa_iirfilters_descriptor = {
  CLAP_VERSION,
  "skei.audio/sa_iirfilters",
  "sa_iirfilters",
  "skei.audio",
...
//------------------------------
  ...
  MIP_IIRFilter MLFilter    = {};
  MIP_IIRFilter MRFilter    = {};

//------------------------------
public:
//------------------------------

  sa_iirfilters_plugin(const clap_plugin_descriptor_t* ADescriptor, const clap_host_t* AHost)
...
//------------------------------
  ...
  void recalc(float srate) {
    MLFilter.init(MSampleRate);
    MRFilter.init(MSampleRate);
    MNeedRecalc = false;
  }
...
//------------------------------
  ...
  bool init() final {
    ...
    if (result) {
      appendStereoInput();
      appendStereoOutput();
      appendParameter( new MIP_TextParameter( "Mode", 0,      0,   11, filter_types_txt ));
      appendParameter( new MIP_Parameter(     "Freq", 1024,   50,  16000 ));
      appendParameter( new MIP_Parameter(     "Q",    0.707,  0.1, 1 ));
      appendParameter( new MIP_Parameter(     "Gain", 0,      -50, 50 ));
      MNeedRecalc = true;
    }
    return result;
  }
...
//----------------------------------------------------------------------
  ...
  MIP_DEFAULT_ENTRY(sa_iirfilters_descriptor,sa_iirfilters_plugin)
